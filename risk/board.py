...
class Board(object):
    ...
    def is_valid_path(self, path):
        ...
        1. For all territories V in the list (except the last one), the
        ...
        if len(path) <= 0:
            ...
            for i in range(len(path) - 1):
                ...
                if path[i] in path[i + 1:]:
                    ret &= False
                elif path[i + 1] not in neighbors:
                    ret &= False
        return ret
    ...
    def is_valid_attack_path(self, path):
        ...
        a player's armies cannot move through territories they already
        occupy;
        they must move through enemy territories.
        All valid attacks, therefore, will follow a path of starting on
        one player's territory and moving trough enemy territories.
    ...
    def cost_of_attack_path(self, path):
        ...
        Args:
            path ([int]): a list of territory_ids which must be a valid
            attack path
    ...
    def shortest_path(self, source, target):
        ...
        board = [territory.territory_id for territory in self.data]

        if source == target:
            return stack

        if target in risk.definitions.territory_neighbors[source]:
            stack.append(target)
            return stack

        while len(queue) > 0:
            next_stack = queue.popleft()
            neighbors = risk.definitions.territory_neighbors[next_stack[-1]]
            for territory in board:
                if territory in neighbors:
                    if territory == target:
                        return next_stack + [territory]
                    elif territory not in next_stack:
    ...
    def can_fortify(self, source, target):
        ...
        Returns:
            bool: True if reinforcing the target from the source territory
            is a valid move
        '''
        stack = []
        stack.append(source)
        ...
        for territory in self.data:
            if territory.player_id == self.owner(source):
                board.append(territory.territory_id)

        if source == target:
            return True

        elif target not in board:
            return False
    ...
    def cheapest_attack_path(self, source, target):
        ...
        if source == target:
            return None
        for territory in self.data:
            board[territory.territory_id] = territory.armies
        dictionary = {}
            ...
            for territory in board:
                if territory not in visited_territories:
                    if territory in neighbors:
                        ...
                        prio = currenttuple[-1] + board[territory]
                        prio_names = [ele[0] for ele in prio_queue]
                        if territory not in prio_names:
                            dictionary[territory] = copy
                            prio_queue.append((territory, prio))
                        for i in range(len(prio_queue)):
                            if prio_queue[i][0] == territory:
                                if prio < prio_queue[i][-1]:
                                    dictionary[territory] = copy
                                    prio_queue[i] = (territory, prio)
                        prio_queue.sort(key=lambda x: x[-1])
            visited_territories.append(current)
    ...
    def owns_continent(self, player_id, continent_id):
        ...
        """
        return all((t.player_id == player_id for t in
    ...
    def num_foreign_continent_territories(self, continent_id, player_id):
        """
        Compute the number of territories owned by other players on a
        given continent.
        ...
        Returns:
            ...
            players.
        """
        return sum(1 if t.player_id != player_id else 0 for
                   t in self.continent(continent_id))
    ...
    def possible_attacks(self, player_id):
        ...
        return [Move(from_t.territory_id, from_t.armies,
                ...
                to_t in self.hostile_neighbors(from_t.territory_id)]
    ...
    def fortify(self, from_territory, to_territory, n_armies):
        ...
        if n_armies < 0 or self.armies(from_territory) <= n_armies:
            raise ValueError('Board: Cannot move {n} armies from territory\
                             {tid}.'
                             .format(n=n_armies, tid=from_territory))
        if to_territory not in [t.territory_id for t in
                                self.friendly_neighbors(from_territory)]:
            raise ValueError('Board: Cannot fortify, territories do not\
                             share owner and/or border.')
    ...
    def attack(self, from_territory, to_territory, attackers):
        ...
        if attackers < 1 or self.armies(from_territory) <= attackers:
            raise ValueError('Board: Cannot attack with {n} armies\
                             from territory {tid}.'
                             .format(n=attackers, tid=from_territory))
        if to_territory not in [tid for (tid, _, _) in
                                self.hostile_neighbors(from_territory)]:
    ...
    def fight(cls, attackers, defenders):
        ...
        Returns:
            tuple (int, int): Number of lost attackers, number of
            lost defenders.
        """
        n_attack_dices = min(attackers, 3)
        n_defend_dices = min(defenders, 2)
        attack_dices = sorted([cls.throw_dice() for _ in
                              range(n_attack_dices)], reverse=True)
        defend_dices = sorted([cls.throw_dice() for _ in
                              range(n_defend_dices)], reverse=True)
    ...
    def set_armies(self, territory_id, n):
        ...
        """
        if n < 1:
            raise ValueError('Board: cannot set the number of armies to\
                    <1 ({tid}, {n}).'.format(tid=territory_id, n=n))
    ...
    def territories_of(self, player_id):
        ...
        """
        return [t.territory_id for t in self.data
